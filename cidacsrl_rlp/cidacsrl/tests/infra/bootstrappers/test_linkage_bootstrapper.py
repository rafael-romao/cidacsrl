...
@pytest.fixture
def mock_configs():
    """Gera instâncias mockadas das configurações carregadas via YAML."""
    workflow_config = MagicMock()
    workflow_config.linkage_config_path = "/mock/path/linkage_config.yml"
    workflow_config.es_config_path = "/mock/path/elasticsearch_config.yml"
    workflow_config.source_format = "parquet"
    workflow_config.output_format = "parquet"
    
    blocking_config = MagicMock()
    blocking_config.source_table = "internacao_example"
    blocking_config.target_es_index = "nascimentos_example"
    
    es_config = MagicMock()
    
    return workflow_config, blocking_config, es_config


class TestLinkageBootstrapper:
    @patch("cidacsrl_rlp.cidacsrl.infra.bootstrappers.linkage_bootstrapper.load_linkage_workflow_config")
    @patch("cidacsrl_rlp.cidacsrl.infra.bootstrappers.linkage_bootstrapper.load_sequential_blocking_workflow_config")
    ...
    @patch("cidacsrl_rlp.cidacsrl.infra.bootstrappers.linkage_bootstrapper.RunSequentialLinkageWorkflowUseCase")
    def test_bootstrap_sequential_linkage_success(
        ...
        workflow_config, blocking_config, es_config = mock_configs
        mock_load_linkage.return_value = workflow_config
        mock_load_seq.return_value = blocking_config
        ...
        mock_load_linkage.assert_called_once_with("/fake/path.yml")
        mock_load_seq.assert_called_once_with(workflow_config.linkage_config_path)
        mock_load_es.assert_called_once_with(workflow_config.es_config_path)
        mock_get_es_client.assert_called_once_with(es_config, use_cache=False)
        mock_repo_adapter_cls.assert_called_once_with(spark_session=mock_spark_session, env_config=workflow_config)

        mock_repo_instance.check_health.assert_called_once_with(
            blocking_config.source_table, blocking_config.target_es_index
        )
        ...
        mock_use_case_instance.execute.assert_called_once_with(blocking_config)
    ...
    def test_bootstrap_fails_on_unhealthy_infrastructure(
        ...
        """Caminho de Falha: Força o erro de diagnóstico (check_health) no FileSystem."""
        workflow_config, blocking_config, es_config = mock_configs
        mock_load_linkage.return_value = workflow_config
        mock_load_seq.return_value = blocking_config
        mock_load_es.return_value = es_config
