...
def spark():
    session = SparkSession.builder \
        .appName("cidacsrl-test-data-ingestion") \
        .master("local[2]") \
...
def test_check_health_success(spark, mock_source_config):
...
def test_check_health_input_missing_returns_error(spark, mock_source_config):
    ingestion_adapter = make_ingestion_adapter(spark, mock_source_config)
    errors = ingestion_adapter.check_health(source_table="tabela_inexistente")
    assert len(errors) == 1
    assert "Erro ao acessar dados de origem" in errors[0]

def test_read_source_data(spark, mock_source_config):
    ingestion_adapter = make_ingestion_adapter(spark, mock_source_config)
