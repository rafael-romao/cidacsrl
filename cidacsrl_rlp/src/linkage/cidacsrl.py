...
def cidacsrl_v2(
    ...
    spark,
    es_settings,
    intermediate_results_base_path,
    partition: str = "*",
    log_file: str = None,
    logger=None
):
    ...
    # Keep original schema for final output construction
    original_source_schema = df.schema
    ...
    for phase_config in workflow_config.blocking_phases:
        ...
        current_source_count_for_phase = df.select(workflow_config.id_source_table).count()
        ...
        df_strong_matches_this_phase = execute_linkage_phase(
            spark,
            df, # Current set of source records to link
            workflow_config,
            phase_config,
            es_settings,
            # current_partition_value, # FIXME não teremos mais esse argumento, o código que chama essa função já precisa passar o DF filtrado ou a partição em questão
            logger=logger
        )
        ...
        if strong_matches_count_this_phase > 0:
            ...
            # Colunas de auditoria
            df_strong_matches_to_write = df_strong_matches_to_write.withColumns({
                "_LINKED_FROM": F.lit(phase_config.phase_name),
                "_DT_LINKAGE": F.from_utc_timestamp(F.current_timestamp(), "America/Sao_Paulo"),
            })
            ...
            if partition == "*":
                ...
                # Remove a coluna particionada dos dados caso ela tenha sido mantida. Se o linkage for partitionado ela terá apenas o valor da partição nos campos.
                df_strong_matches_to_write = df_strong_matches_to_write.drop(workflow_config.partition_by['partition'])
                write_path = f"{re.sub("\/$", "", str(phase_output_path))}/{workflow_config.partition_by['partition']}={partition}"

            df_strong_matches_to_write.write.format("parquet").mode("overwrite").save(write_path)
            logger.info(f"Strong match candidates from phase '{phase_config.phase_name}' written successfully to: {phase_output_path}")

            # Identify unique source IDs that found a strong match in this phase
            ids_matched_in_phase = spark.read.parquet(write_path).select(workflow_config.id_source_table).distinct()
        ...
        if df.count() == 0 and phase_config != workflow_config.blocking_phases[-1]:
