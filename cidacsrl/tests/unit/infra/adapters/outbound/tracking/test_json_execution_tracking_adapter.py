...
from cidacsrl.cidacsrl.domain.models.tracking.work_unit import WorkUnitMetadata, WorkUnitStatus, WorkUnitStatus, WorkUnitExecutionRecord
...
def test_initialize_job_state_creates_new_json_file(adapter, tracking_dir):
    ...
    checkpoint_file = Path(tracking_dir) / f"job_{job_id}_state.json"
...
def test_update_work_unit_status_transitions_correctly(adapter, tracking_dir):
    ...
    # Modifica o status para PROCESSING e valida a persistência atômica em disco
